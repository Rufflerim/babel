...
namespace pimpl_details {
    ...
    template<class T, class D, class C = pimpl_details::default_copier_t<T>>
...
template<class T, class Deleter = pimpl_details::default_deleter_t<T>>
class upimpl {
protected:
    static_assert(!std::is_array<T>::value, "upimpl specialization for arrays is not implemented");
    struct dummy_t_ {
        int dummy__;
    };
...
public:
    ...
    using is_default_manageable = pimpl_details::is_default_manageable<T, deleter_type>;

    SPIMPL_CONSTEXPR upimpl() SPIMPL_NOEXCEPT
            : ptr_(nullptr, deleter_type {}) {}

    SPIMPL_CONSTEXPR upimpl(std::nullptr_t) SPIMPL_NOEXCEPT
            : upimpl() {}

    template<class D>
    upimpl(pointer p, D&& d,
    ...
    upimpl(U* u,
           typename std::enable_if<
                   ...
                   dummy_t_
           >::type = dummy_t_()) SPIMPL_NOEXCEPT
            : upimpl(u, &pimpl_details::default_delete<T>, &pimpl_details::default_copy<T>) {}

#ifndef SPIMPL_NO_CPP11_DEFAULT_MOVE_SPEC_FUNC

    upimpl(upimpl&& r) SPIMPL_NOEXCEPT = default;

#else
    upimpl(upimpl&& r) SPIMPL_NOEXCEPT
    : ptr_(std::move(r.ptr_)) {}
#endif

#ifdef SPIMPL_HAS_AUTO_PTR
    template<class U>
    upimpl(std::auto_ptr<U>&& u,
             typename std::enable_if<
                std::is_convertible<U*, pointer>::value
                    && is_default_manageable::value,
                dummy_t_
             >::type = dummy_t_()) SPIMPL_NOEXCEPT
    : ptr_(u.release(), &pimpl_details::default_delete<T>) {}
#endif

    template<class U>
    upimpl(std::unique_ptr<U>&& u,
           typename std::enable_if<
                   std::is_convertible<U*, pointer>::value
                   && is_default_manageable::value,
                   dummy_t_
    ...
    upimpl(const upimpl<T, Deleter>&) = delete;

#ifndef SPIMPL_NO_CPP11_DEFAULT_MOVE_SPEC_FUNC

    upimpl& operator=(upimpl&& r) SPIMPL_NOEXCEPT = default;

#else
    upimpl& operator= (upimpl&& r) SPIMPL_NOEXCEPT {
        ptr_ = std::move(r.ptr_);
        return *this;
    }
#endif
    ...
    {
        return operator=(upimpl(std::move(u)));
    }
#endif
    ...
    reference operator=(const upimpl<T, Deleter>&) = delete;
    ...
    void swap(upimpl& u) SPIMPL_NOEXCEPT {
        using std::swap;
        ptr_.swap(u.ptr_);
    }
    ...
    get_deleter() const SPIMPL_NOEXCEPT { return ptr_.get_deleter(); }
...
inline void swap(upimpl<T, D>& l, upimpl<T, D>& r) SPIMPL_NOEXCEPT {
    l.swap(r);
}


template<class T1, class D1, class T2, class D2>
inline bool operator==(const upimpl<T1, D1>& l, const upimpl<T2, D2>& r) {
    return l.get() == r.get();
}
...
template<class T1, class D1, class T2, class D2>
inline bool operator<(const upimpl<T1, D1>& l, const upimpl<T2, D2>& r) {
    using P1 = typename upimpl<T1, D1>::pointer;
    using P2 = typename upimpl<T2, D2>::pointer;
...
template<class T, class D>
inline bool operator==(const upimpl<T, D>& p, std::nullptr_t) SPIMPL_NOEXCEPT {
    return !p;
}

template<class T, class D>
inline bool operator==(std::nullptr_t, const upimpl<T, D>& p) SPIMPL_NOEXCEPT {
    return !p;
}
...
template<class T, class D>
inline bool operator<(const upimpl<T, D>& l, std::nullptr_t) {
    using P = typename upimpl<T, D>::pointer;
    return std::less<P>()(l.get(), nullptr);
}

template<class T, class D>
inline bool operator<(std::nullptr_t, const upimpl<T, D>& p) {
    using P = typename upimpl<T, D>::pointer;
    return std::less<P>()(nullptr, p.get());
}

template<class T, class D>
inline bool operator>(const upimpl<T, D>& p, std::nullptr_t) {
    return nullptr < p;
}
...
inline upimpl<T> make_unique_impl(Args&& ... args) {
    return upimpl<T>(new T(std::forward<Args>(args)...), &pimpl_details::default_delete<T>);
}


// Helpers to manage unique impl, stored in std::unique_ptr
template<class T, class Deleter = pimpl_details::default_deleter_t<T>, class Copier = pimpl_details::default_copier_t<T>>
class impl_ptr : public upimpl<T, Deleter> {
    using base_type = upimpl<T, Deleter>;
...
public:
    ...
    using is_default_manageable = pimpl_details::is_default_manageable<T, deleter_type, copier_type>;
    ...
    impl_ptr(U* u,
             typename std::enable_if<
                     ...
                     dummy_t_
             >::type = dummy_t_()) SPIMPL_NOEXCEPT
            : impl_ptr(u, &pimpl_details::default_delete<T>, &pimpl_details::default_copy<T>) {}
    ...
    impl_ptr(std::auto_ptr<U>&& u,
             typename std::enable_if<
                ...
                dummy_t_
             >::type = dummy_t_()) SPIMPL_NOEXCEPT
    : base_type(u.release(), &pimpl_details::default_delete<T>), copier_(&pimpl_details::default_copy<T>) {}
#endif
    ...
    impl_ptr(std::unique_ptr<U>&& u,
             typename std::enable_if<
                     ...
                     dummy_t_
             >::type = dummy_t_()) SPIMPL_NOEXCEPT
            : base_type(u.release(), &pimpl_details::default_delete<T>) {}
...
inline impl_ptr<T> make_impl(Args&& ... args) {
    return impl_ptr<T>(new T(std::forward<Args>(args)...), &pimpl_details::default_delete<T>,
                       &pimpl_details::default_copy<T>);
}
...
namespace std {
    template<class T, class D>
    struct hash<spimpl::upimpl < T, D>> {
    using argument_type = spimpl::upimpl<T, D>;
